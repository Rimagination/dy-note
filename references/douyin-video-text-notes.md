...
2. Browser video extraction: Douyin URL -> web-access Chrome tab -> video URL -> local media file -> 16 kHz WAV -> Whisper or Qwen3-ASR -> clean text outputs.
...
- `doubao_brief.md` and `doubao_brief.json`: fast search/visual hypothesis with evidence level.
...
- Reuse `doubao_brief.json` unless the full share text changed or the task now requires `evidence` mode.
...
- `Doubao -> local ASR`: only when Doubao is blocked/weak, when exact wording matters, or when the user wants durable transcript material.
...
- `hypothesis`: plausible pattern from Doubao or a small sample.
...
| `script-mining` | User wants script拆解, 镜头拆解, hook, rhythm, storyboard, or remix material | Doubao visual hypothesis + ASR; keyframes when visual claims matter | Hook library, scene sequence, narration structure, reusable script template |
...
When multiple modes apply, run the cheapest pass that can answer the first decision. For example, an account analysis can start with titles and Doubao briefs, then choose only the best or most ambiguous videos for ASR.
...
Use this route when the user wants a fast "what is this video about" answer, a rough timeline, or a draft for writing. It must use the current web-access Chrome session and verify that Doubao Web is logged in before submitting anything.
...
6. `## 关键细节与证据`: cite transcript, Doubao brief, comments, or keyframes without overloading the note.
...
Doubao is useful for a fast visual/story hypothesis, especially when full share text triggers search-backed interpretation. Treat visual details as hypotheses unless confirmed with screenshots/keyframes. For script mining, combine:
...
- `inspect_workflow_state.py` should identify reusable artifacts before rerunning browser, Doubao, ASR, comment fetch, budget, or score stages.
