...
- 画面意识：抖音视频常常没有可提取字幕；如果长视频转写很少，DyNote 会提醒你补豆包画面理解、关键帧或 OCR。
...
- 只是想知道视频讲什么、做选题筛选或先拿草稿：默认优先用已登录豆包网页快读。
- 要详细笔记、可靠原文、逐句内容、引用、脚本拆解或事实核查：优先复用已有字幕/文本；没有时再做本地自动语音识别。
- 视频较长但转写文本明显很少：说明信息可能主要在画面、贴纸文字、操作步骤、商品细节或氛围里，需要补豆包 evidence、关键帧或 OCR。DyNote 不会把一份很稀疏的音频转写当成完整视频理解。
