...
DyNote 是一个面向 Codex / Agent 的抖音内容分析 skill。它会把抖音分享文案、视频链接、账号主页、话题关键词或评论样本，整理成有来源、有证据、可检索、能继续写作和研究的 Markdown 材料。

它的核心特点是：

- 视频到笔记：把单条视频整理成学习笔记、脚本拆解、选题素材或事实核查简报。
- 评论洞察：把评论里的需求、痛点、反对意见、购买顾虑和高频反馈整理成可用结论，而不是只抓一份评论列表。
- 账号与话题研究：围绕账号主页、话题关键词或竞品视频，分析定位、栏目结构、钩子模板和选题模式。
- 先快后稳：优先用已登录抖音网页版的“问AI / 识别画面”快速理解视频；需要可靠原文、引用或拆解时，再升级到本地自动语音识别、关键帧或评论样本。
- 画面意识：抖音视频常常没有可提取字幕；如果长视频转写很少，DyNote 会提醒你补抖音内置 AI 画面理解、关键帧或 OCR。
- 避免返工：先检查已有 `douyin_ai_brief.md`、`doubao_brief.md`、`transcript.txt`、`segments.json`、`metadata.json` 和 `note_budget.json`，只补真正缺的证据。

## 输出内容

一次完整分析通常包含两层结果：给人读的主笔记，以及给复核、续写和追问用的原始材料包。主笔记会按证据预算组织学习收获、核心判断、脚本结构、方法步骤、评论洞察和局限；材料包保存抖音内置 AI 概述、备用豆包概述、转写文本、结构化片段、元数据、评论样本、预算和评分。
...
- `doubao_brief.md` / `doubao_brief.json`：备用豆包快速概述和证据等级
...
| 登录浏览器 | 抖音内置 AI、备用豆包快读、抖音页面加载 | Chrome、`web-access`、当前 Chrome 已登录对应网页 | 未登录时停止，并提示先登录 |
...
DyNote 会先看你的目标，再决定要不要转写音频：

- 只是想知道视频讲什么、做选题筛选或先拿草稿：默认优先用已登录抖音网页版的“问AI / 识别画面”。
- 要详细笔记、可靠原文、逐句内容、引用、脚本拆解或事实核查：优先复用已有字幕/文本；没有时再做本地自动语音识别。
- 视频较长但转写文本明显很少：说明信息可能主要在画面、贴纸文字、操作步骤、商品细节或氛围里，需要补抖音内置 AI 画面理解、关键帧或 OCR。DyNote 不会把一份很稀疏的音频转写当成完整视频理解。

抖音字幕也分两种：能开关的字幕更可能是独立字幕轨，可以整理成文本；已经压在画面里的字幕或贴纸文字没有独立文件，不能靠音频转写读取。
...
抖音内置 AI 很适合快速理解视频，尤其是页面已有“章节要点”时；`识别画面` 可以把当前帧加入问答上下文。需要可靠原文时，DyNote 会回落到本地自动语音识别或关键帧检查。豆包现在只是备用路线，如果结果是搜索派生内容，DyNote 不会把它当成“已经逐帧看过视频”。

## 登录和隐私

抖音内置 AI 路线需要你已经在当前 Chrome 登录抖音网页版。备用豆包路线需要你已经在当前 Chrome 登录豆包网页版。DyNote 只让浏览器自己加载页面，不会导出或保存 Cookie、localStorage、token、签名参数等登录凭据。
...
- 抖音内置 AI 和备用豆包快读适合做草稿、筛选和视觉假设；发布级结论要用本地自动语音识别、关键帧、评论或外部来源补证。
...
- `scripts/compute_note_budget.py`：按转写长度、时长、评论量和互动质量生成 `note_budget.json`，并提示长视频转写过稀时的画面依赖风险。
