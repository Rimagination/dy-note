...
- 用户只是问“这个视频讲什么”、想做选题筛选、草稿或快速理解时，默认优先走已登录豆包 `fast` 快读。已有 `doubao_brief.json` 时先复用。
...
- 用户要镜头、画面文字、贴纸文字、操作步骤、商品/价格/场景细节时，不能只靠音频转写；需要豆包 `evidence` 模式、关键帧、截图或 OCR。
...
- `single-video-note`：默认模式。单条视频/分享文本 -> 先按任务意图选择豆包快读、字幕/ASR 全文、关键帧/OCR或组合证据，输出可读笔记。
...
- `quick-pass`：分享文本、页面元数据、豆包 `fast`；适合秒级判断、选题筛选和草稿，是用户未要求详细时的默认起点。
...
3. `evidence-ladder`：把证据分为用户输入、页面元数据、豆包快读、独立字幕轨/ASR 转写、评论、关键帧/OCR、外部来源。结论必须标注依赖哪一层。
...
7. 如果豆包结果是 `blocked`、`weak`，或用户要求可靠全文/字幕/逐句内容，再使用 `extract_douyin_text.py` 打开页面、抓视频源、抽音频并调用本地自动语音识别。默认 `--asr-backend auto`：中文或未指定语言优先共享 Qwen3-ASR，明确外语视频优先 Whisper。默认复用已有输出；需要重跑时加 `--force`。
8. 评论、账号、话题、竞品或批量研究任务先做 `quick-pass` 样本，不要直接对大量视频逐条 ASR；样本结论不足时再升级到 `evidence-pass` 或 `research-pass`。
9. 输出完成后，先打开 `analysis_plan.json`、`transcript.cleaned.md` 或 `doubao_brief.md` 检查证据是否足以回答研究问题。再打开 `note_budget.json`，按推荐长度和 `writing_guidance` 写学习型笔记；如果 `visual_dependency.needs_visual_review=true`，必须在笔记和回复中提醒画面证据不足，或先补豆包 `evidence`/关键帧/OCR。
...
- 如果播放器/页面没有独立字幕轨，画面内嵌字幕、贴纸文字和商品/价格信息不能靠 ASR 读取；需要关键帧、OCR、豆包视觉假设或人工检查。
