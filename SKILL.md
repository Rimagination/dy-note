---
name: dy-note
description: "DyNote: systematically and efficiently extract raw Douyin/DY video data and analyze videos, comments, accounts, hashtags, and short-video scenes into evidence-graded learning notes, summaries, research briefs, scripts, and knowledge-base material. Use when the user asks to 抓取/提取/整理 抖音视频字幕、视频文案、ASR 转写、Qwen3-ASR 中文转写、原始材料归档、学习笔记、analysis plan、note budget、避免返工、复用已有素材、评论洞察、账号分析、赛道/话题研究、竞品拆解、脚本拆解、电商/本地生活视频分析、事实核查、自动搜索素材, save Douyin content as Markdown/TXT, or use logged-in Douyin Web built-in AI / Doubao fallback to quickly interpret a Douyin video/share link before falling back to local ASR/keyframe workflows."
---

# DyNote

DyNote 是面向 Codex / Agent 的抖音视频分析与笔记 skill。把抖音视频变成可继续做学习笔记、总结和写作的干净素材。默认目标不是字幕工程文件，而是先落一份原始数据包：`douyin_ai_brief.md`、`douyin_ai_brief.json`、`transcript.cleaned.md`、`transcript.txt`、`segments.json`、`metadata.json`、`note_budget.json`；当用户想快速理解视频内容时，默认先用已登录抖音网页版的“问AI / 识别画面”，豆包只作为备用快读路线。
...
- 抖音内置 AI 路线需要当前 Chrome 已登录抖音网页版。打开视频后优先使用页面右侧 `问AI`，必要时点击可见的 `识别画面` 把当前帧加入问答上下文。
- 如果抖音页面没有 `问AI` / `识别画面` 或未生成 `章节要点`，记录为 `weak` 或 `blocked`，再考虑豆包备用、本地自动语音识别、关键帧或 OCR。
...
- 用户只是问“这个视频讲什么”、想做选题筛选、草稿或快速理解时，默认优先走已登录抖音网页版内置 AI。已有 `douyin_ai_brief.json` 时先复用；如果不可用，再用豆包 `fast` 备用。
- 用户明确要详细笔记、可靠原文、逐句内容、引用、脚本拆解、事实核查或可发布材料时，先找已有 SRT/VTT/TXT；没有可用字幕轨或转写时，再用本地自动语音识别。中文或未指定语言优先共享 Qwen3-ASR，明确外语视频再用 Whisper 系后端。
- 用户要镜头、画面文字、贴纸文字、操作步骤、商品/价格/场景细节时，不能只靠音频转写；需要抖音 `识别画面`、关键帧、截图或 OCR，豆包 `evidence` 只作为备用视觉假设。
...
如果视频较长，但 `note_budget.json` 中 `visual_dependency.risk` 为 `medium` 或 `high`，必须提醒用户：转写文本过少，完整理解可能依赖画面，不能把稀疏 ASR 写成完整笔记。

## 场景模式路由

先判断用户真正要完成的任务，再决定证据深度和工具路线：

- `single-video-note`：默认模式。单条视频/分享文本 -> 先按任务意图选择抖音内置 AI 快读、备用豆包、字幕/ASR 全文、关键帧/OCR或组合证据，输出可读笔记。
...
- `script-mining`：拆脚本、镜头、叙事节奏、开头钩子、转场、结尾 CTA；抖音内置 AI 或备用豆包可先给画面假设，重要结论要用 ASR/抽帧校验。
...
- `quick-pass`：分享文本、页面元数据、抖音内置 AI，必要时备用豆包 `fast`；适合秒级判断、选题筛选和草稿，是用户未要求详细时的默认起点。
...
3. `evidence-ladder`：把证据分为用户输入、页面元数据、抖音内置 AI、备用豆包快读、独立字幕轨/ASR 转写、评论、关键帧/OCR、外部来源。结论必须标注依赖哪一层。
...
- 默认先提取并保存原始材料，再写总结。不要只把抖音内置 AI、豆包概述或 ASR 文本直接当最终笔记。
...
- 互动质量是“值得多写”的辅助信号，不替代证据。扩写必须来自视频原文、抖音内置 AI/豆包快读、评论样本、关键帧或外部核验。
...
6. 如果用户要快速理解“这个视频讲什么”、做选题筛选或先拿草稿，且当前 Chrome 已登录抖音网页版，优先跑 `douyin_web_ai_brief.py`；已有可用 `douyin_ai_brief.json` 时先读旧结果。抖音内置 AI 不可用时，再用 `doubao_video_brief.py` 的 `fast` 模式备用。
7. 如果抖音内置 AI / 豆包结果是 `blocked`、`weak`，或用户要求可靠全文/字幕/逐句内容，再使用 `extract_douyin_text.py` 打开页面、抓视频源、抽音频并调用本地自动语音识别。默认 `--asr-backend auto`：中文或未指定语言优先共享 Qwen3-ASR，明确外语视频优先 Whisper。默认复用已有输出；需要重跑时加 `--force`。
8. 评论、账号、话题、竞品或批量研究任务先做 `quick-pass` 样本，不要直接对大量视频逐条 ASR；样本结论不足时再升级到 `evidence-pass` 或 `research-pass`。
9. 输出完成后，先打开 `analysis_plan.json`、`douyin_ai_brief.md`、`transcript.cleaned.md` 或 `doubao_brief.md` 检查证据是否足以回答研究问题。再打开 `note_budget.json`，按推荐长度和 `writing_guidance` 写学习型笔记；如果 `visual_dependency.needs_visual_review=true`，必须在笔记和回复中提醒画面证据不足，或先补抖音 `识别画面`/关键帧/OCR。
...
- 抖音内置 AI 很适合快速得到章节要点和时间线；`识别画面` 只能证明当前帧被加入输入框，不能自动等同于完整逐帧分析。
- 豆包 `fast` 模式现在是备用路线；如果结果包含“搜索 N 个关键词 / 参考 N 篇资料”，按检索式解读使用，不要称为“豆包已看完关键帧”。
- 豆包 `blocked` 结果通常会写“无法访问视频画面”或类似措辞；这时不要继续追问编造，直接回落到本地 ASR/关键帧补充。
- 抖音页面的 `caption`、简介和话题标签不等于完整字幕。只有完整逐句文本才可称为字幕/转写。
- 如果播放器/页面没有独立字幕轨，画面内嵌字幕、贴纸文字和商品/价格信息不能靠 ASR 读取；需要抖音 `识别画面`、关键帧、OCR、备用豆包视觉假设或人工检查。
